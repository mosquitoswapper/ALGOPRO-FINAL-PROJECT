...
class Player:
    def __init__(self, map):
        ...
        self.moveSpeed = 1.5
        self.rotationSpeed = 1.5 * (math.pi / 180)
    ...
    def update(self):
        ...
        next_x = self.x + math.cos(self.rotationAngle) * moveStep
        next_y = self.y + math.sin(self.rotationAngle) * moveStep

        if not self.map.has_wall_at(next_x, self.y):
            self.x = next_x
        if not self.map.has_wall_at(self.x, next_y):
            self.y = next_y
